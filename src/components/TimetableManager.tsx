...
import { Plus, Trash2, Clock, Upload, X, Calendar } from "lucide-react";
...
const TimetableManager = ({ uid, email }: Props) => {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(ttRef, (snap) => {
      const data = snap.val();
      if (data) {
        setTimetable(Object.values(data));
      } else {
        setTimetable([]);
      }
    });
  ...
  const currentPeriod = useMemo(() => {
    const now = new Date();
    const dayName = DAYS[now.getDay() - 1]; // 0=Sun
      ...
      {tab === "my" && (
        <>
            ...
            {showAdd && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select value={day} onChange={e => setDay(e.target.value)} className="py-2 px-3 rounded-lg bg-muted/50 border border-border text-foreground text-sm">
                  ...
                  <input
                    ...
                    placeholder="Period"
                    className="py-2 px-3 rounded-lg bg-muted/50 border border-border text-foreground text-sm"
                  />
                  <input value={className} onChange={e => setClassName(e.target.value)} placeholder="Class (e.g. 10A)"
                    className="py-2 px-3 rounded-lg bg-muted/50 border border-border text-foreground text-sm" />
            ...
            <div className="space-y-1.5">
              ...
              {todayEntries.map((e, i) => (
                ...
                >
                  ...
                  <button onClick={() => removeEntry(e.id)} className="p-1.5 text-muted-foreground hover:text-destructive">
          ...
          {DAYS.filter(d => timetable.some(e => e.day === d)).map(d => {
            ...
            const now = new Date();
            const isToday = DAYS[now.getDay() - 1] === d;
              ...
              <div key={d} className="space-y-1.5">
                ...
                {dayEntries.map(e => (
                  <div key={e.id} className="glass-card-light p-3 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-secondary/20 flex items-center justify-center text-secondary text-xs font-bold">
                      P{e.period}
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium text-foreground">{e.subject}</p>
          ...
          {timetable.length === 0 && (
            <div className="text-center py-12 text-muted-foreground text-sm">
              No timetable entries yet. Tap + to add periods.
            </div>
          )}
        </>
      )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {admin ? "Admin · You can add exams" : "View only"}
              </p>
            </div>
            {admin && (
          ...
          <AnimatePresence>
            {showExamAdd && admin && (
              ...
              >
                ...
                <input value={examSubject} onChange={e => setExamSubject(e.target.value)} placeholder="Subject"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input type="time" value={examStartTime} onChange={e => setExamStartTime(e.target.value)}
                    className="py-2 px-3 rounded-lg bg-muted/50 border border-border text-foreground text-sm" />
                  <input type="number" min="30" max="300" value={examDuration} onChange={e => setExamDuration(Number(e.target.value))}
            ...
            {exams.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((e, i) => {
              ...
              return (
                ...
                >
                  ...
                  {admin && (
                    <button onClick={() => removeExam(e.id)} className="p-1.5 text-muted-foreground hover:text-destructive">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  )}
                </motion.div>
